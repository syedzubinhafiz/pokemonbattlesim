...
class TestTask2(TesterBase):
    ...
    def test_limit_2(self):
        try:
            ...
            self.verificationErrors.append(f"Dawn's team could not be instantiated: {str(e)}.")
            return
        try:
            with captured_output("1 2 3") as (inp, out, err):
                # So 1 2 3 is a valid entry and should be the correct team.
                team.choose_team(0, None)
        except Exception as e:
            self.verificationErrors.append(f"Dawn's team could not be chosen: {str(e)}.")
            return
        output = out.getvalue().strip()
        ...
        try:
            assert str(
                team) == "Charmander's HP = 7 and level = 1, Bulbasaur's HP = 9 and level = 1, Bulbasaur's HP = 9 and level = 1, Squirtle's HP = 8 and level = 1, Squirtle's HP = 8 and level = 1, Squirtle's HP = 8 and level = 1"
        except AssertionError:
            self.verificationErrors.append(f"PokeTeam does not handle limit correctly. {str(team)} ")
